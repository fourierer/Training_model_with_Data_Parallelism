...
parser = argparse.ArgumentParser(description='Bloody Image Training')
...
#        model = models.__dict__[args.arch]()
    ...
    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[gpu + 4]) # 使用后面几块GPU
...
def train(train_loader, model, criterion, optimizer, epoch, gpu, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        acc1, acc2 = accuracy(output, target, topk=(1, 5))
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    lr = args.lr * (1 - epoch / args.epochs)  # 学习率从0.4开始线性衰减，再训练240个epochs
